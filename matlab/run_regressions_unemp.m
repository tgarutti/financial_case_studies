...
X_pi = [Lt(window(3):window(end)) 100*inflation(window(2):window(end-1))...
     ...
     -100*unemployment(window(3):window(end))];
...
X_y = [Eyt(window(4):window(end)) 100*outputGap(window(2):window(end-2)) ...
    ...
    (100*nairu(window(3):window(end-1))-100*unemployment(window(3):window(end-1)))];
...
E_nu_t = 100*unemployment + normrnd(0,1,size(unemployment));
X_nu = [E_nu_t(window(4):window(end)) ... 
    100*unemployment(window(2):window(end-2)) ...
    100*unemployment(window(1):window(end-3))];
Y_nu = 100*unemployment(window(3):window(end-1));

[coefnu, ~, residualnu] = lsqlin(X_nu, Y_nu, [], []);
coefnu = [coefnu(1) 1-coefnu(1) coefnu(2)/(1-coefnu(1)) ...
          coefnu(3)/(1-coefnu(1))];
residualnu = residualnu';

%Regression of nairu
E_nustar_t = 100*nairu + normrnd(0,1,size(nairu));
X_nustar = [E_nustar_t(window(4):window(end)) ... 
    100*nairu(window(2):window(end-2)) ...
    100*nairu(window(1):window(end-3))];
Y_nustar = 100*nairu(window(3):window(end-1));
...
res_unemployment = [res_unemployment; residualnu];
res_nairu = [res_nairu; residualnustar];
