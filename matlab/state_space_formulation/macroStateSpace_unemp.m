...
% x_t = Q*xi_t + H1*z_{t-1} + H2*z_{t-2} + H3*Ft + H4*nu_t + H5*nustar_t +
%       J*w_t, where 
...
global z PCOrtec_window unemp nair_u
%% Define variables
Gamma = rho(:,:,i);
Omega = sigma(:,:,i);
...
initialEstimates = [Pi(1,1),Pi(1,2),Pi(2,1),Pi(2,2),R(1,1),R(1,2),R(2,1),R(2,2),...
    ...
    H3(3,8),H3(3,9),H3(3,10),H4(2,1),H4(3,1),H5(3,1)];
...
[ML_parameters(i,:),ML_LogL(i)] = fmincon('MVNegativeLogLikelihood_unemp',...
